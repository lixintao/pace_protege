...
/*
Our ModificationItem
*/
	public void addRelation(String name)  //Add a relation to ontology where argument passed is URI of relation 给本体加一个关系，通过路径进行传递
	{
	...
	public void removeRelation(String name) //Remove a relation from ontology. Argument passed is URI of relation. 从本体中删除一种关系，参数传递是关系的URI
	{
		if(name.split("#").length >= 2){
	...
	public OWLRelation getRelation(String name) //Return relation already stored in ontology. Argument passed is URI of relation. Return null if that relation don't exists        in this ontology 取本体中的关系
	{
		if(name.split("#").length >= 2){
	...
	public boolean doesContainRelation(String name) //Check if this ontology contains a relation whose URI is passed 检查本体中是否包含这种关系，通过URI进行判断
	{
		if(name.split("#").length >= 2){
